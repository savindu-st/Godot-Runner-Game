...
func _run():
	var anim_path = "res://models/Leonard/jump.res"
	var anim = load(anim_path) as Animation
	if not anim:
		print("Could not load jump.res")
		return
		
	print("Fixing root motion in jump.res...")
	...
	if hips_track == -1:
		print("Could not find Hips position track in jump.res")
		return
	...
	for k in range(key_count):
		...
		# We'll lock X and Z, allowing the character to jump up (Y axis)
		var new_pos = Vector3(0.0, pos.y, 0.0)
		anim.track_set_key_value(hips_track, k, new_pos)
		
	ResourceSaver.save(anim, anim_path)
	print("Successfully fixed root motion! jump.res is now in-place.")
